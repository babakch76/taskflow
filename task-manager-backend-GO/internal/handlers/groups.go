...
// UpdateMemberRole handles PATCH /groups/{group_id}/members/{user_id}/role.
//
// Owner-only. Promotes a member to manager ("admin") or demotes them back.
// The owner's own row is untouchable: there is exactly one owner, fixed at
// creation, and transferring that is out of scope — the same position
// LeaveGroup takes when an owner tries to leave.
func (h *GroupHandler) UpdateMemberRole(w http.ResponseWriter, r *http.Request) {
	...
	callerRole, err := h.DB.GetMemberRole(groupID, callerID)
	if err != nil {
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	}
	if callerRole != RoleOwner {
		jsonError(w, "only the group owner can change member roles", http.StatusForbidden)
		return
	}
	if targetID == callerID {
		jsonError(w, "you cannot change your own role", http.StatusForbidden)
		return
	}

	targetRole, err := h.DB.GetMemberRole(groupID, targetID)
	if err != nil {
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	}
	if targetRole == "" {
		jsonError(w, "user is not a member of this group", http.StatusNotFound)
		return
	}
	if targetRole == RoleOwner {
		// Unreachable while there is one owner and they cannot target
		// themselves, but stated explicitly so the invariant survives any
		// future ownership-transfer work.
		jsonError(w, "the group owner's role cannot be changed", http.StatusConflict)
		return
	}
	if targetRole == req.Role {
		jsonResponse(w, http.StatusOK, map[string]string{"message": "role unchanged"})
		return
	}
	...
	// Guard on the role we read, so a concurrent change loses rather than
	// being silently overwritten — same rows-affected pattern as the invites.
	res, err := tx.Exec(
		`UPDATE group_members SET role = ? WHERE group_id = ? AND user_id = ? AND role = ?`,
		req.Role, groupID, targetID, targetRole,
	)
	if err != nil {
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	...
	if affected, _ := res.RowsAffected(); affected == 0 {
		jsonError(w, "member's role changed since you loaded it, try again", http.StatusConflict)
		return
	}

	var username string
	if err := tx.QueryRow(`SELECT username FROM users WHERE id = ?`, targetID).Scan(&username); err != nil {
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	}

	detail := username + " → " + req.Role
	if err := recordActivity(tx, groupID, callerID, EventMemberRoleChanged, nil, detail); err != nil {
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	}
