...
// --- C4: bulk status update -------------------------------------------------

func TestBulkUpdateTaskStatus(t *testing.T) {
	db := newTestDB(t)
	h := &TaskHandler{DB: db}

	owner := newUser(t, db, "owner")
	groupID := newGroup(t, db, owner, "Study Group")
	a := newTask(t, db, groupID, "a")
	b := newTask(t, db, groupID, "b")
	untouched := newTask(t, db, groupID, "c")

	body := `{"task_ids":["` + a + `","` + b + `"],"status":"done"}`
	rec := httptest.NewRecorder()
	h.BulkUpdateTaskStatus(rec, request(
		http.MethodPatch, "/groups/x/tasks", body, owner,
		map[string]string{"group_id": groupID},
	))
	...
	if len(got) != 2 {
		t.Fatalf("expected 2 updated tasks, got %d", len(got))
	}
	for _, task := range got {
		...
		if task.UpdatedAt == nil {
			t.Errorf("task %s: updated_at was not set", task.ID)
		}
	}

	var status string
	if err := db.QueryRow(`SELECT status FROM tasks WHERE id = ?`, untouched).Scan(&status); err != nil {
		t.Fatalf("read untouched task: %v", err)
	}
	if status != "todo" {
		t.Fatalf("unselected task changed status to %q", status)
	}

	// Exactly one summary event, not one per task.
	var events int
	if err := db.QueryRow(
		`SELECT COUNT(*) FROM activity_events WHERE group_id = ? AND event_type = ?`,
		groupID, EventTasksBulkUpdated,
	).Scan(&events); err != nil {
		t.Fatalf("count events: %v", err)
	}
	if events != 1 {
		t.Fatalf("expected 1 bulk activity event, got %d", events)
	}
}

func TestBulkUpdateTaskStatusRejectsBadInput(t *testing.T) {
	db := newTestDB(t)
	h := &TaskHandler{DB: db}
	...
	foreign := newTask(t, db, otherGroup, "not yours")
	...
	cases := []struct {
		...
		{"empty task_ids", `{"task_ids":[],"status":"done"}`, http.StatusBadRequest},
		{"missing task_ids", `{"status":"done"}`, http.StatusBadRequest},
		{"invalid status", `{"task_ids":["` + foreign + `"],"status":"finished"}`, http.StatusBadRequest},
		{"task from another group", `{"task_ids":["` + foreign + `"],"status":"done"}`, http.StatusNotFound},
		{"unknown task id", `{"task_ids":["nope"],"status":"done"}`, http.StatusNotFound},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			rec := httptest.NewRecorder()
			h.BulkUpdateTaskStatus(rec, request(
				http.MethodPatch, "/groups/x/tasks", tc.body, owner,
				map[string]string{"group_id": groupID},
			))
			if rec.Code != tc.want {
	...
	// The rejected cross-group request must not have touched the other group.
	var status string
	if err := db.QueryRow(`SELECT status FROM tasks WHERE id = ?`, foreign).Scan(&status); err != nil {
		t.Fatalf("read foreign task: %v", err)
	}
	if status != "todo" {
		t.Fatalf("task in another group was modified (status=%q)", status)
	}
}
